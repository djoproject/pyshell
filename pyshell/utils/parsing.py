...
#
# # RULE 1 ## <commands>  ::= <command> <threading> <EOL> | <command> "|"
#                             <commands>
# # RULE 2 ## <threading> ::= "&" | ""
...
class Parser(list):
    """
    This object will parse a command line withou any resolution
    of process, argument, or parameter
    """
    ...
    def _pushCommandInList(self):
        if len(self.currentCommand) > 0:
            self.append((tuple(self.currentCommand),
                         tuple(self.argSpotted),
                         tuple(self.paramSpotted),))
    ...
    def _parse(self, char):
        ...
        if self.escapeChar:
            ...
            self.lastBackground = (len(self),
                                   len(self.currentCommand),
                                   len(self.currentToken))
            self.currentToken += char
        else:
            self.currentToken += char
