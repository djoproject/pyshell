...
class EngineCoreTest(unittest.TestCase):
    ...
    def testInit(self):
        ...
        mc.dymamic_count = 42
        e = EngineV3([mc], [[]], [[{},{},{}]])
        self.assertIs(e.cmd_list[0],mc)
        self.assertEqual(mc.dymamic_count, 0) #check the call on reset

        mc.addProcess(noneFun,noneFun,noneFun) #because the reset with the dynamic at 42 will remove every command...

        #empty dict
        self.assertIsInstance(e.env,ParameterContainer)
    ...
    def testExecuteSimpleOne(self):
        ...
        self.assertEqual(self.pro_count,1)
        ...
        self.assertEqual(self.pro_count,1)
        self.assertEqual(self.post_count,3)
    ...
    def testSimpleOneWithMultiOutput(self):
        ...
        self.engine.execute()
        self.assertEqual(self.pre_count,1)
        self.assertEqual(self.pro_count,2)
        self.assertEqual(self.post_count,4)
        ...
        self.engine.execute()
        self.assertEqual(self.pre_count,3)
        self.assertEqual(self.pro_count,4)
        self.assertEqual(self.post_count,24)

        self.valueToTest = [225, 3375, 400, 8000, 625, 15625, 900, 27000]
        self.pre_count = self.pro_count = self.post_count = 0
        self.engine = EngineV3([uc, uc, uc], [[], [], []], [[{},{},{}], [{},{},{}], [{},{},{}]])
        self.engine.execute()
        self.assertEqual(self.pre_count,7)
        self.assertEqual(self.pro_count,8)
        self.assertEqual(self.post_count,112)
    ...
    def testMultiOuputAndMultiCommandAmdCommandLimit(self):
        ...
        self.assertIs(self.pre_count[0],3)
        self.assertIs(self.pro_count[0],6)
        self.assertIs(self.post_count[0],12)
        self.assertIs(self.pre_count[1],0)
        self.assertIs(self.pro_count[1],0)
        self.assertIs(self.post_count[1],0)
        ...
        self.assertIs(self.pre_count[0],0)
        self.assertIs(self.pro_count[0],0)
        self.assertIs(self.post_count[0],0)
        self.assertIs(self.pre_count[1],3)
        self.assertIs(self.pro_count[1],6)
        self.assertIs(self.post_count[1],12)
        ...
        self.assertIs(self.pre_count[0],9)
        self.assertIs(self.pro_count[0],12)
        self.assertIs(self.post_count[0],120)

        self.assertIs(self.pre_count[1],6)
        self.assertIs(self.pro_count[1],12)
        self.assertIs(self.post_count[1],24)
    ...
    def checkStack(self,stack,cmd_list):
        for data,path,typ,enablingMap in stack:
            ...
            #check path
            self.assertTrue(len(path) >0 and len(path) <= len(cmd_list))
            for i in range(0,len(path)):
                self.assertTrue(path[i] >= 0 and path[i] < len(cmd_list[i]))
    ...
    def test_limitReaching(self):
        ...
        self.assertEqual(uc[0][0].pre_count,256)
        self.assertEqual(uc[0][0].pro_count,255)
        self.assertEqual(uc[0][0].post_count,255)

        engine = EngineV3([uc], [[]], [[{},{},{}]])
        engine.stack[0] = ([5]*(DEFAULT_EXECUTION_LIMIT+1),[0],1,None )
        self.assertRaises(ExecutionException, engine.execute)
        self.assertEqual(uc[0][0].pre_count,0)
        self.assertEqual(uc[0][0].pro_count,256)
        self.assertEqual(uc[0][0].post_count,255)
        ...
        self.assertRaises(ExecutionException, engine.execute)
        self.assertEqual(uc[0][0].pre_count,0)
        self.assertEqual(uc[0][0].pro_count,0)
        self.assertEqual(uc[0][0].post_count,256)
